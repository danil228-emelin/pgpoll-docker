...
if [ -z "$(ls -A $PGDATA)" ]; then
    ...
    until pg_isready -h node_b -p 5432 -U replicator; do
    ...
    pg_basebackup -h node_b -p 5432 -U replicator -D "$PGDATA" -v -P --wal-method=stream -R
    ...
    echo "primary_conninfo = 'host=node_b port=5432 user=replicator password=replicator_pass application_name=node_c'" > $PGDATA/postgresql.auto.conf
